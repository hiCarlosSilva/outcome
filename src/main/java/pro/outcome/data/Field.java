...
public class Field<T> {
	...
	Field(Model parent, Class<T> cl, String name, boolean indexed, ValueGenerator<T> def, Constraint ... constraints) {
		...
		// We need to delay fetching the related entity to prevent initialisation order problems:
		_related = null;
	}
	...
	T toObject(Object value) {
		...
		if(_foreignKey) {
			if(_related == null) {
				...
				Entity<?> f = (Entity<?>)Entities.getEntity(m.getEntityName());
				System.out.println("Facade: "+f);
				_related = (Entity<?>)Entities.getEntity(Entities.getModel(_type.getSimpleName()).getEntityName());
			}
